...
: "${VALIDATION_REPOSITORY:?VALIDATION_REPOSITORY must be set}"
: "${VALIDATION_REF:?VALIDATION_REF must be set}"
: "${GITHUB_ACTION_PATH:?GITHUB_ACTION_PATH must be set}"

if [[ ! "$VALIDATION_REPOSITORY" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ ]]; then
  echo "VALIDATION_REPOSITORY must use owner/repository" >&2
  exit 1
fi
...
git clone --depth 1 --filter=blob:none --sparse \
  ...
  "https://github.com/$VALIDATION_REPOSITORY.git" "$source_repo"
